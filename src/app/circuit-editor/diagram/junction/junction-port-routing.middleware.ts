...
// state.
export function createJunctionPortRoutingMiddleware(): Middleware<'junction-port-routing'> {
  return {
    name: 'junction-port-routing',
    execute(context, next) {
      ...
      const junctions = state.nodes.filter((n) => n.type === JUNCTION_NODE_TYPE);
