...
// Reshape handles on every orthogonal segment of a selected wire. Dragging
// flips the wire to `routingMode: 'manual'`; first/last segments keep their
// port ends anchored, and dragging a junction-adjacent segment moves the
// junction along with the partner half of a former-parent split.
@Component({
...
export class EdgeReshapeOverlayComponent {
  ...
  private classifyEndpoint(nodeId: string): EndpointKind {
    ...
    if (node?.type === JUNCTION_NODE_TYPE) return 'junction';
    return 'port';
  }
  ...
  private snapshotPropagation(
    ...
    if (!junction || junction.type !== JUNCTION_NODE_TYPE) return null;
  ...
  private anchorEndpointToPort(
    ...
    const node = this.modelService.getNodeById(nodeId);
    const anchor = portWorldPosition(node ?? null, portId);
    if (!anchor) return;
