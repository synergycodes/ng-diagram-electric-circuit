// When a reshape handle on a junction-adjacent segment is dragged, the junction
// (and the collinear partner half of the same split) should follow, so the wire
// reshapes as if it were never split. These helpers find the partner segment and
...
// Returns the partner's segment when its junction-adjacent segment is collinear
// with the dragged segment (same axis + fixed coord within tolerance), else null.
...
// False when either edge lacks a formerParentId — independent branches must not
// propagate as a half pair.
export function sharesFormerParent(a: { data?: unknown }, b: { data?: unknown }): boolean {
  const aFormerParent = (a.data as CircuitEdgeData | undefined)?.formerParentId;
  const bFormerParent = (b.data as CircuitEdgeData | undefined)?.formerParentId;
  return !!aFormerParent && aFormerParent === bFormerParent;
}

// Uses the junction-side endpoint (not the segment-start vertex) so the delta
