...
const buildDoc = (): DxfDocument => {
  ...
  doc.addLayer(new DxfLayer('COMPONENTS', 7));
  doc.addLayer(new DxfLayer('WIRES', 7));
  doc.addLayer(new DxfLayer('LABELS', 7));
  ...
  doc.addEntity(new DxfText('LABELS', 'R1', 51, 184, 4.2, 'BOLD', 1, 2));
...
describe('DxfWriter AutoCAD skeleton', () => {
  ...
  it('always defines layer 0 alongside the document layers', () => {
    ...
    expect(layers).toEqual(['0', 'COMPONENTS', 'WIRES', 'LABELS']);
  });
